...
public class AutomationService {
    ...
    private void handleTaskRelatedToTemperatureSensor(FeedGroup group) throws IOException {
        ...
        if (currentTemperature < threshold[0]
...
//      ));
        ...
        } else if (currentTemperature > threshold[1]
                && group.getLight().getCurrentValueAsInt() != 0) {
//      log.warn(String.format(
...
//    }
    ...
    private void handleTaskRelatedToLCDMonitor(FeedGroup group) throws IOException {
        ...
        String noOfActiveDays = group.getNumberOfActiveDay() == -1 ? "-1" : group.getNumberOfActiveDay().toString();
        String hatchedEgg = (
        ...
        String currentTemp = (
                ...
                ? group.getTemperatureSensor().getCurrentValue() : "-1";
        ...
        if (group.getScreen() != null) {
            adafruitService.addFeedValue(
                    group.getScreen().getKey(),
                    CustomStringUtils.encodeStringToIntString(message)
            );
        }
    }
